...
const TransitionRoadmap = () => {
  const { planResult, telemetry, planInput, setPlanResult } =
    useSimulatorStore();
  ...
  const actualSeries = useMemo(() => {
    ...
    const mean =
      telemetry.reduce((a, b) => a + b.facility_energy_kWh, 0) /
      telemetry.length;
    let cum = 0;
  ...
  function handleAddActuals(phaseId: string, actuals: PhaseActuals) {
    ...
    const updated = {
      ...planResult,
      phases: planResult.phases.map((p) =>
        p.id === phaseId ? { ...p, actuals, status: "done" as const } : p
      ),
    };
  ...
  function handleReplanFromHere(index: number) {
    ...
    const totalActualCO2 = completedPhases.reduce(
      (sum, p) => sum + (p.actuals?.realizedCO2DeltaKg ?? 0),
      0
    );
    const remainingTarget = planResult.totals.capexUSD * 0.8 - totalActualCO2;
    console.log(
      `[v0] Re-optimizing from phase ${index + 1}, remaining target: ${remainingTarget}`
    );
  }
    ...
    return (
      <div className="p-6 text-center">
        <p className="text-neutral-400">
          Generate a roadmap using Auto-Plan in the left panel.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-4">
            ...
            {planResult.expectedSeries?.length ? (
              <polyline
                ...
                points={(() => {
                  const w = 800;
                  const h = 140;
                  const xs = planResult.expectedSeries.map((d) =>
                    d.time.getTime()
                  );
                  const ys = planResult.expectedSeries.map(
                    (d) => d.cumulativeEnergy_kWh
                  );
                  ...
                  const minY = 0,
                    maxY = Math.max(1, ...ys);
                  const sx = (x: number) =>
                    ((x - minX) / Math.max(1, maxX - minX)) * w;
                  const sy = (y: number) =>
                    h - ((y - minY) / Math.max(1, maxY - minY)) * h;
                  return planResult.expectedSeries
                    .map(
                      (d) =>
                        `${sx(d.time.getTime())},${sy(d.cumulativeEnergy_kWh)}`
                    )
                    .join(" ");
                })()}
              />
        ...
        <p className="text-xs text-neutral-500 mt-3">
          Expected is derived from phase end dates and projected annual energy
          deltas; Actual is a proxy from telemetry.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-4">
        ...
        <p className="text-xs text-neutral-400 mb-3">
          ...
          <strong>phased capital deployment model</strong> that optimizes for:
        </p>
        <ul className="text-xs text-neutral-400 space-y-2 ml-4 list-disc">
          <li>
            <strong>Levelized Cost of Energy (LCOE):</strong> Prioritizes
            technologies with lowest long-term cost per kWh.
          </li>
          <li>
            <strong>Capacity Factor:</strong> Selects renewables best suited to
            your region's climate and geography.
          </li>
          <li>
            <strong>Payback Period:</strong> Ensures each phase achieves
            positive ROI within your target timeline.
          </li>
          <li>
            <strong>Permitting Lead Times:</strong> Accounts for regulatory
            delays, ensuring realistic schedules.
          </li>
          <li>
            <strong>Energy Storage & Resilience:</strong> Adds battery storage
            to smooth intermittency and improve grid stability.
          </li>
        </ul>
        <p className="text-xs text-neutral-400 mt-3">
          As you record actuals, the plan recalibrates to reflect real-world
          performance, tariff changes, and market conditions.
        </p>
      </div>
      ...
      <PhaseDetailDrawer
        ...
        onOpenChange={setPhaseDrawerOpen}
      />
    </div>
  );
};
